...
const UpdateVehicules = () => {
  ...
  const fetchVehicle = async () => {
    ...
    try {
      ...
      setAssignedDate(vehicle.assignedDate ? vehicle.assignedDate.split('T')[0] : '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Données envoyées pour mise à jour:', vehicleData);
      const response = await instance.put(`/vehicules/${id}`, vehicleData);

      console.log('Véhicule modifié avec succès', response.data);
      setSuccessMessage('Véhicule modifié avec succès !');
      
      // Rediriger vers la liste des véhicules après 1.5 secondes
      setTimeout(() => {
      ...
      if (error.response?.data?.message) {
        ...
        setErrorMessage(['Une erreur est survenue lors de la modification du véhicule']);
      }
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="bg-gradient-to-r from-green-600 to-emerald-600 px-6 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-green-100 text-sm">Modifier les informations du véhicule</p>
              </div>
            </div>
          </div>
