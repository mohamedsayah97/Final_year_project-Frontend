...
const ListeVehicules = () => {
  ...
  const handleDelete = useCallback(
    async (id: string, registrationNumber: string) => {
      if (
        window.confirm(
          `Êtes-vous sûr de vouloir supprimer le véhicule "${registrationNumber}" ?`,
        )
      ) {
        setIsDeleting(id);
        try {
  ...
  const getVehicleTypeIcon = useCallback((type: string) => {
              ...
              <>
                ...
                {totalPages > 1 && (
                  <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-6">
                    <div className="text-sm text-gray-500">
                      Affichage de {indexOfFirstVehicle + 1} à{" "}
